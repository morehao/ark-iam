...
import { getApplicationPageList, getTenantPageList } from '@ark-iam/api'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [apps, tenants] = await Promise.allSettled([
        ...
        >
          ...
          <Row gutter={[16, 16]}>
            {[
              ...
              { icon: <FileSearchOutlined />, title: '平台治理', desc: '菜单字典 · 自定义域名' },
            ].map((f) => (
