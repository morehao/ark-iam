...
- 租户管理：租户 CRUD（类型/挂起）
- 租户应用：租户订阅关系管理
- API密钥监督：全租户 API Key 只读监督（归属租户/归属主体/创建人/状态；无写操作，密钥生命周期在租户控制台管理）
- 域名管理：域名 CRUD + 验证状态
...
- 用户管理：Tab「用户」= 真实用户 CRUD/部门/角色；Tab「服务账号」= 创建/编辑/挂起/删除/角色授权（不可登录、不入组织，仅作授权主体与 API Key 归属；详情内嵌其密钥只读列表）
- 角色管理：角色 CRUD、菜单授权、成员分配
- API密钥：Tab「我的密钥」= 个人密钥自助管理（代表用户本人）；Tab「服务账号密钥」= 服务端集成密钥管理（归属服务账号，需系统管理能力），创建即展示明文一次
- 租户动态菜单树由 `/v1/tenant/menus/tree` 下发
