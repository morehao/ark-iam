...
test.describe('OIDC SSO E2E', () => {
  test('RP1 首次登录：访问 租户管理后台 → 自动跳转 login-web → 填写凭证 → 回调展示首页', async ({ page }) => {
    await rp1Login(page);
  });
  ...
  test('Admin 登录后 → 租户管理后台免密登录（同一 context：共享 SSO session）', async ({ page, context }) => {
    await adminDirectLogin(page);
  ...
  test('Admin 全局登出后 → 租户管理后台需重新认证', async ({ page, context }) => {
    await adminDirectLogin(page);
    const rp1Page = await context.newPage();
    await rp1SSOLogin(rp1Page);
