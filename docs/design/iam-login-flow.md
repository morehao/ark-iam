...
| 7 | **Connector SSO 登录** | 外部 IdP → IAM（IAM 作为 RP）| OAuth2/OIDC | `/v1/auth/connector/:id/authorize`、`/v1/auth/connector/callback` | 进入 OIDC 主流程 |
...
```
1. 前端 POST /v1/auth/connector/:connectorId/authorize
   → 驱动按 protocol 构建授权 URL, state + nonce 存 Redis (TTL 10min)
   → 302/?authorizationUrl 跳外部 IdP
2. 用户在外部门户授权 → 302 回调 /v1/auth/connector/callback?code&state
