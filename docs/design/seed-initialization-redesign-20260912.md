...
> ⚠️ **矩阵取值已修订**：本文「产品身份 → Reconcile」的划线过宽，导致控制台整页不可编辑；reconcile 的准入判据已收窄为「定位键 + 安全不变式」，见 [seed-authority-scope-revision-20260912.md](seed-authority-scope-revision-20260912.md)。**模型（三分语义、矩阵唯一真相源、控制台拒写、值匹配迁移）不变，本文其余结论仍然有效。**
