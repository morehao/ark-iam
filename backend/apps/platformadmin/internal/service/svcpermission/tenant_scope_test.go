...
func TestMenuTreeUsesAppID(t *testing.T) {
	...
	if err := db.Create(&model.MenuEntity{AppID: 10, Name: "root10", ParentID: 0}).Error; err != nil {
		t.Fatalf("seed app10 root: %v", err)
	}
	if err := db.Create(&model.MenuEntity{AppID: 20, Name: "root20", ParentID: 0}).Error; err != nil {
