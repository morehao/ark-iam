...
func TestDepartmentTreeUsesContextTenant(t *testing.T) {
	...
	if err := db.Create(&model.DepartmentEntity{TenantID: "34", ParentID: "", Name: "root34"}).Error; err != nil {
		t.Fatalf("seed tenant 34 department: %v", err)
	}
	if err := db.Create(&model.DepartmentEntity{TenantID: "99", ParentID: "", Name: "root99"}).Error; err != nil {
		t.Fatalf("seed tenant 99 department: %v", err)
	}
