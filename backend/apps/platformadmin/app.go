...
func Init(engine *gin.Engine, Conf *pkgconfig.Config) {
	...
	if Conf != nil {
		...
		oidcAuthOpts = append(oidcAuthOpts, oidcauth.WithOIDCAudiences("platform-admin-web"))
	}
...
func registerBackChannelLogout(engine *gin.Engine, Conf *pkgconfig.Config, getOIDCPublicKey func() *rsa.PublicKey) {
	...
	logout.RegisterReceiverRoutes(group, basePath, getOIDCPublicKey, Conf.OIDC.Issuer, "platform-admin-web", nil)
}
