...
func initTrace() error {
	provider, err := otlptracegrpc.NewGRPCProvider(context.Background(), tenantadmin.AppName, config.Conf.Server.Env, config.Conf.Trace)
	if err != nil {
		glog.Errorf(context.Background(), "[%s.initTrace] init trace failed, fallback to disabled mode, err:%v", tenantadmin.AppName, err)
		return nil
	}
...
func shutdownTraceProvider() {
	...
	if err := traceProvider.Shutdown(ctx); err != nil {
		glog.Errorf(context.Background(), "[%s.shutdownTraceProvider] shutdown fail, err:%v", tenantadmin.AppName, err)
	}
}
