	...
	if Conf != nil && Conf.OIDC.EnableSSOSessionValidation {
		...
		oidcAuthOpts = append(oidcAuthOpts,
			oidcauth.WithOIDCSSOValidation(func(ctx *gin.Context, personID uint, isMachineToken bool) bool {
				...
				if err != nil {
					glog.Warnf(ctx, "[tenantadmin.Init] HasActiveSession fail, personID:%d, err:%v", personID, err)
					return true
				}
				return active
			}))
...
func registerBackChannelLogout(engine *gin.Engine, Conf *pkgconfig.Config, getOIDCPublicKey func() *rsa.PublicKey) {
	...
	logout.RegisterReceiverRoutes(group, basePath, getOIDCPublicKey, Conf.OIDC.Issuer, "tenant-admin-web", nil)
}
