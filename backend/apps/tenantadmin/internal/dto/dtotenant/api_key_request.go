...
// 租户端 API 密钥请求 DTO（归属真实用户本人或服务账号）。

type ApiKeyPageListReq struct {
	Page          int    `json:"page" form:"page"`                   // 页码
	PageSize      int    `json:"pageSize" form:"pageSize"`           // 每页数量
	Name          string `json:"name" form:"name"`                   // 名称(模糊)
	MachineUserID string `json:"machineUserID" form:"machineUserID"` // 归属服务账号ID(空=当前真实用户本人)
	All           bool   `json:"all" form:"all"`                     // 是否查看租户全部密钥(需系统管理能力)
}

type ApiKeyCreateReq struct {
	Name          string `json:"name" binding:"required"` // 密钥名称
	MachineUserID string `json:"machineUserID"`           // 归属服务账号ID(空=代表当前真实用户本人)
