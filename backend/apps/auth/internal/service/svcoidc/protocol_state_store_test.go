...
func TestCompleteAuthRequest(t *testing.T) {
	...
	err = store.CompleteAuthRequest(context.Background(), req.GetID(), "person:88", authTime, []string{"pwd"}, "", 0, true)
	require.NoError(t, err)
...
func TestSaveAndConsumeCode(t *testing.T) {
	...
	_ = store.CompleteAuthRequest(context.Background(), req.GetID(), "person:88", time.Now(), []string{"pwd"}, "", 0, true)
