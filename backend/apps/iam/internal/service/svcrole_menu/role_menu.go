...
func (svc *roleMenuSvc) Create(ctx *gin.Context, req *dtorole.RoleMenuCreateReq) (*dtorole.RoleMenuCreateResp, error) {
	...
	if err := dao.NewRoleMenuDao().Insert(ctx, insertEntity); err != nil {
		glog.Errorf(ctx, "[svcrole_menu.Create] dao Insert fail, err:%v, req:%s", err, gutil.ToJsonString(req))
...
func (svc *roleMenuSvc) Delete(ctx *gin.Context, req *dtorole.RoleMenuDeleteReq) error {
	...
	if err != nil {
		glog.Errorf(ctx, "[svcrole_menu.Delete] dao GetByID fail, err:%v, req:%s", err, gutil.ToJsonString(req))
	...
	if err := dao.NewRoleMenuDao().Delete(ctx, req.RoleID, userID); err != nil {
		glog.Errorf(ctx, "[svcrole_menu.Delete] dao Delete fail, err:%v, req:%s", err, gutil.ToJsonString(req))
		return code.GetError(code.RoleMenuDeleteError)
	}
	return nil
}
...
func (svc *roleMenuSvc) PageList(ctx *gin.Context, req *dtorole.RoleMenuPageListReq) (*dtorole.RoleMenuPageListResp, error) {
	...
	if err != nil {
		glog.Errorf(ctx, "[svcrole_menu.PageList] dao GetPageListByCond fail, err:%v, req:%s", err, gutil.ToJsonString(req))
