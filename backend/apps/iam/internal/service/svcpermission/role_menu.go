...
func (svc *roleMenuSvc) Create(ctx *gin.Context, req *dtopermission.RoleMenuCreateReq) (*dtopermission.RoleMenuCreateResp, error) {
	...
	if err != nil {
		glog.Errorf(ctx, "[svcpermissionmenu.Create] dao GetByID fail, err:%v, req:%s", err, gutil.ToJsonString(req))
	...
	if err := dao.NewRoleMenuDao().Insert(ctx, insertEntity); err != nil {
		glog.Errorf(ctx, "[svcpermissionmenu.Create] dao Insert fail, err:%v, req:%s", err, gutil.ToJsonString(req))
...
func (svc *roleMenuSvc) Delete(ctx *gin.Context, req *dtopermission.RoleMenuDeleteReq) error {
	...
	if err != nil {
		glog.Errorf(ctx, "[svcpermissionmenu.Delete] dao GetByID fail, err:%v, req:%s", err, gutil.ToJsonString(req))
	...
	if err := dao.NewRoleMenuDao().Delete(ctx, req.RoleID, userID); err != nil {
		glog.Errorf(ctx, "[svcpermissionmenu.Delete] dao Delete fail, err:%v, req:%s", err, gutil.ToJsonString(req))
		return code.GetError(code.RoleMenuDeleteError)
	}
	return nil
}
...
func (svc *roleMenuSvc) PageList(ctx *gin.Context, req *dtopermission.RoleMenuPageListReq) (*dtopermission.RoleMenuPageListResp, error) {
	...
	if err != nil {
		glog.Errorf(ctx, "[svcpermissionmenu.PageList] dao GetPageListByCond fail, err:%v, req:%s", err, gutil.ToJsonString(req))
