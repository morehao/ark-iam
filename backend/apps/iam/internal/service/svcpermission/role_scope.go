...
func (svc *roleScopeSvc) Create(ctx *gin.Context, req *dtopermission.RoleScopeCreateReq) (*dtopermission.RoleScopeCreateResp, error) {
	...
	if err != nil {
		glog.Errorf(ctx, "[svcpermissionscope.Create] dao GetByID fail, err:%v, req:%s", err, gutil.ToJsonString(req))
	...
	if err := dao.NewRoleScopeDao().Insert(ctx, insertEntity); err != nil {
		glog.Errorf(ctx, "[svcpermissionscope.Create] dao Insert fail, err:%v, req:%s", err, gutil.ToJsonString(req))
...
func (svc *roleScopeSvc) Delete(ctx *gin.Context, req *dtopermission.RoleScopeDeleteReq) error {
	...
	if err != nil {
		glog.Errorf(ctx, "[svcpermissionscope.Delete] dao GetByID fail, err:%v, req:%s", err, gutil.ToJsonString(req))
	...
	if err := dao.NewRoleScopeDao().Delete(ctx, req.RoleID, userID); err != nil {
		glog.Errorf(ctx, "[svcpermissionscope.Delete] dao Delete fail, err:%v, req:%s", err, gutil.ToJsonString(req))
		return code.GetError(code.RoleScopeDeleteError)
	}
	return nil
}
...
func (svc *roleScopeSvc) PageList(ctx *gin.Context, req *dtopermission.RoleScopePageListReq) (*dtopermission.RoleScopePageListResp, error) {
	...
	if err != nil {
		glog.Errorf(ctx, "[svcpermissionscope.PageList] dao GetPageListByCond fail, err:%v, req:%s", err, gutil.ToJsonString(req))
