...
type UserRoleSvc interface {
	Create(ctx *gin.Context, req *dtorole.UserRoleCreateReq) (*dtorole.UserRoleCreateResp, error)
	Delete(ctx *gin.Context, req *dtorole.UserRoleDeleteReq) error
	PageList(ctx *gin.Context, req *dtorole.UserRolePageListReq) (*dtorole.UserRolePageListResp, error)
}
...
func (svc *userRoleSvc) Create(ctx *gin.Context, req *dtorole.UserRoleCreateReq) (*dtorole.UserRoleCreateResp, error) {
	...
	if err := dao.NewUserRoleDao().Insert(ctx, insertEntity); err != nil {
		glog.Errorf(ctx, "[svcuser_role.Create] dao Insert fail, err:%v, req:%s", err, gutil.ToJsonString(req))
		return nil, code.GetError(code.UserRoleCreateError)
	}
	return &dtorole.UserRoleCreateResp{}, nil
}

func (svc *userRoleSvc) Delete(ctx *gin.Context, req *dtorole.UserRoleDeleteReq) error {
	userRoleEntity, err := dao.NewUserRoleDao().GetByID(ctx, req.UserID)
	if err != nil {
		glog.Errorf(ctx, "[svcuser_role.Delete] dao GetByID fail, err:%v, req:%s", err, gutil.ToJsonString(req))
	...
	if err := dao.NewUserRoleDao().Delete(ctx, req.UserID, userID); err != nil {
		glog.Errorf(ctx, "[svcuser_role.Delete] dao Delete fail, err:%v, req:%s", err, gutil.ToJsonString(req))
		return code.GetError(code.UserRoleDeleteError)
	}
	return nil
}

func (svc *userRoleSvc) PageList(ctx *gin.Context, req *dtorole.UserRolePageListReq) (*dtorole.UserRolePageListResp, error) {
	...
	if err != nil {
		glog.Errorf(ctx, "[svcuser_role.PageList] dao GetPageListByCond fail, err:%v, req:%s", err, gutil.ToJsonString(req))
		return nil, code.GetError(code.UserRoleGetPageListError)
	}

	list := make([]dtorole.UserRolePageListItem, 0, len(userRoleEntityList))
	for _, v := range userRoleEntityList {
		list = append(list, dtorole.UserRolePageListItem{
			UserID:   v.UserID,
			RoleID:   v.RoleID,
			TenantID: v.TenantID,
		})
	}
	return &dtorole.UserRolePageListResp{
		List:  list,
		Total: total,
	}, nil
}