...
type UserRoleEntity struct {
	gormdao.BaseEntity
	TenantID  string `gorm:"column:tenant_id;type:varchar(36);not null;default:'';comment:租户id;uniqueIndex:uk_user_role_active,where:deleted_at IS NULL" json:"tenantID"`
	UserID    string `gorm:"column:user_id;type:varchar(36);not null;default:'';comment:用户ID;uniqueIndex:uk_user_role_active,where:deleted_at IS NULL" json:"userID"`
	RoleID    string `gorm:"column:role_id;type:varchar(36);not null;default:'';comment:角色ID;uniqueIndex:uk_user_role_active,where:deleted_at IS NULL" json:"roleID"`
