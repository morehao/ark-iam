...
type RoleMenuEntity struct {
	gormdao.BaseEntity
	TenantID  string `gorm:"column:tenant_id;type:varchar(36);not null;default:'';comment:租户id;uniqueIndex:uk_role_menu_active,where:deleted_at IS NULL" json:"tenantID"`
	RoleID    string `gorm:"column:role_id;type:varchar(36);not null;default:'';comment:角色ID;uniqueIndex:uk_role_menu_active,where:deleted_at IS NULL" json:"roleID"`
	MenuID    string `gorm:"column:menu_id;type:varchar(36);not null;default:'';comment:菜单ID;uniqueIndex:uk_role_menu_active,where:deleted_at IS NULL" json:"menuID"`
