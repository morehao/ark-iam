package objpermission

import (
	"github.com/morehao/ark-iam/pkg/model"
)

type MenuBaseInfo struct {
	AppID        string                     `json:"appID" form:"appID" binding:"required"` // 应用ID（菜单必须归属某个应用）
	ParentID     string                     `json:"parentID" form:"parentID"`              // 父菜单ID
	Name         string                     `json:"name" form:"name"`                      // 菜单名称
	Code         string                     `json:"code" form:"code"`                      // 菜单编码
	Path         string                     `json:"path" form:"path"`                      // 菜单路径
	Icon         string                     `json:"icon" form:"icon"`                      // 菜单图标
	Sort         int                        `json:"sort" form:"sort"`                      // 排序
	Type         model.MenuType             `json:"type" form:"type"`                      // 菜单类型
	Visibility   model.MenuVisibility       `json:"visibility" form:"visibility"`          // 可见性门槛(public/member/admin)
	Component    string                     `json:"component" form:"component"`            // 组件路径
	Redirect     string                     `json:"redirect" form:"redirect"`              // 重定向路径
	Hidden       model.MenuHiddenFlag       `json:"hidden" form:"hidden"`                  // 是否隐藏(enable/disable)
	ExternalLink model.MenuExternalLinkFlag `json:"externalLink" form:"externalLink"`      // 是否外链(enable/disable)
	KeepAlive    model.MenuKeepAliveFlag    `json:"keepAlive" form:"keepAlive"`            // 是否缓存(enable/disable)
	Status       model.MenuStatus           `json:"status" form:"status"`                  // 状态
}

// MenuItemNode 菜单树公共节点（跨应用复用：tenantadmin / platformadmin 均返回该结构）。
type MenuItemNode struct {
	MenuID string `json:"menuID"` // 菜单ID
	MenuBaseInfo
	Children []MenuItemNode `json:"children"` // 子菜单
}
