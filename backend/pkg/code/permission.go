...
const (
	...
	// ApplicationClientAuthMethodInvalidError 令牌端点认证方式不在白名单（只允许 client_secret_basic/post/none）。
	ApplicationClientAuthMethodInvalidError = 100824
	// ApplicationClientBuiltInAuthMethodImmutableError 内置客户端是浏览器公共客户端，其认证方式与强制 PKCE
	// 属安全不变式（RFC 6749 §10.1 / RFC 10017 §6.3.3.1），控制台拒写；改动会当场锁死对应控制台。
	ApplicationClientBuiltInAuthMethodImmutableError = 100825
	// ApplicationClientPublicClientSecretForbiddenError 公共客户端（token_endpoint_auth_method=none）
	// 不得签发客户端密钥：密钥随产物下发给用户即公开，且令牌端点不校验它（RFC 6749 §10.1）。
	ApplicationClientPublicClientSecretForbiddenError = 100826
...
var permissionErrorMsgMap = gerror.CodeMsgMap{
	...
	ApplicationClientAuthMethodInvalidError:           "令牌端点认证方式不合法（仅支持 client_secret_basic / client_secret_post / none）",
	ApplicationClientBuiltInAuthMethodImmutableError:  "内置OAuth客户端是浏览器公共客户端，其令牌端点认证方式与强制 PKCE 由平台版本定义，不可修改",
	ApplicationClientPublicClientSecretForbiddenError: "公共客户端（token_endpoint_auth_method=none）不支持客户端密钥",
