...
const (
	...
	MenuBuiltInFieldImmutableError = 100606 // 内置应用的菜单：结构与展示字段由平台版本定义，不可修改（可改状态）
)
...
const (
	...
	ApplicationBuiltInFieldImmutableError = 100749 // 内置应用：名称与描述由平台版本定义，不可修改
...
const (
	...
	ApplicationClientBuiltInFieldImmutableError = 100821 // 内置客户端：名称由平台版本定义，不可修改
...
var permissionErrorMsgMap = gerror.CodeMsgMap{
	...
	MenuBuiltInFieldImmutableError:              "内置应用的菜单树由平台版本定义：不可新增/删除，名称/编码/路径等不可修改（状态可改）",
	...
	ApplicationBuiltInFieldImmutableError:       "内置应用的名称与描述由平台版本定义，不可修改（状态/排序可改）",
	...
	ApplicationClientBuiltInFieldImmutableError: "内置OAuth客户端的名称由平台版本定义，不可修改（回调地址等运行参数可改）",
