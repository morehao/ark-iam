// Package logout 提供 OIDC Back-Channel Logout（OIDC Back-Channel Logout 1.0）
// 的 RP 侧接收端能力。
//
// 定位：供各业务应用（RP）挂载 back-channel logout 接收端点，
// 接收由 auth（OP）在用户登出后推送的 logout_token，并执行本地会话清除。
// 与 pkg/iam/sso（OP 侧登出登记与 SLO 队列）对应，本包是"接收端"。
package logout
